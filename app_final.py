# app.py — MTFSA LIVE DASHBOARD (FINAL VERSION)
...
# ---------- CONFIG ----------
BASE_DIR = Path(r"C:\Users\SAANVI\Downloads\MTFSA_AUTOSCALER")
...
region_files = {r: DATA_DIR / f"{r}_d0_to_d30_hourly.parquet" for r in regions}
...
def load_all_data():
    datasets = {}
    for r, path in region_files.items():
        if path.exists():
...
st.markdown(
    ...
    **MTFSA v5** — Pre-trained on 377 dense (R1) | Evaluated on 1,272 sparse (R2–R5)  
    31 days | 3-shot | No fine-tuning | RF refinement | Docker-ready
    """
)